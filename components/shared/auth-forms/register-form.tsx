...
export const RegisterForm = ({ className, onFieldChange }: Props) => {
  ...
  const onSubmit = async (data: TFormRegisterData) => {
    ...
    try {
      ...
      toast({
        title: "Вы успешно отправили заявку на открытие счета.",
        description: `Спасибо что пользуетесь нашими услугами!`,    
      })

      const successUrl = `/register/success?userName=${encodeURIComponent(data.userName)}&telegram=${encodeURIComponent(data.telegram)}&discord=${encodeURIComponent(data.discord)}`;
